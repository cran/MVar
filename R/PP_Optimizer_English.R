PP_Optimizer = function(Data, Class = NA, Findex = "HOLES", DimProj = 2, Sphere = TRUE, 
  ...
  if ((Findex %in% c("MOMENT", "CHI", "FRIEDMANTUKEY", "ENTROPY", "LEGENDRE",
     ...
     stop("For the index 'MOMENT', 'CHI', 'FRIEDMANTUKEY', 'ENTROPY', 'LEGENDRE', 
          'LAGUERREFOURIER', 'HERMITE' and 'NATURALHERMITE', 'DimProj' should be 2 (two). Verify!")

  if (DimProj >= ncol(Data))
     stop("DimProj greater, or equal to the number of columns in Data. Verify!")

  if (!is.logical(Sphere))
     stop("'Sphere' input is incorrect, it should be TRUE or FALSE. Verify!")
  ...
  if (!(OptMethod %in% c("GTSA", "SA")))
     stop("'OptMethod' input is incorrect, it should be of type character, being 'GTSA' or 'SA'. Verify!")

  if (nrow(Data) < ncol(Data) && Sphere)
     stop("For spherical data the number of observations should be greater, or equal to the number of variables in Data. Verify!")

  if (!is.numeric(Cooling) || Cooling <= 0 || Cooling >= 1)
     stop("'Cooling' input is incorrect, it should be a numeric value between (0,1). Verify!")

  if (!is.numeric(Eps) || Eps <= 0 || Eps >= 1)
     stop("'Eps' input is incorrect, it should be a numeric value between (0,1). Verify!")

  if (!is.numeric(Maxiter) || Maxiter < 1 || (floor(Maxiter)-Maxiter) != 0)
     stop("'Maxiter' input is incorrect, it should be a numeric integer greater than zero. Verify!")
  ...
  while (mi <= Maxiter && Cooling > Eps) {
    ...
    print(paste ("Iteration <-", round(mi,1),"   Index <-", round(IndexMax,10), "   Cooling <-", round(Cooling,9)))#,"   i<- ", round(i,1)))
  ...
  } else colnames(Proj.Data) <- c(paste("Projection", 1:(ncol(Proj.Data))))
